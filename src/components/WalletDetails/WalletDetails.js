...
const WalletDetails = ({balance, setIsWalletDetails, setIsDarkMode, isDarkMode, setIsLogout, setIsAudio, isAudio, setIsLeaderboard, counter, setCounter, isNotification, setActiveModal, setIsNotification }) => {
  ...
  return (
    ...
    >
      ...
      <div className="wallet-details-footer">
        ...
        <div className="wallet-details-actions" onClick={() => setIsDarkMode(!isDarkMode)}>
      ...
      {isNetwork && <NetworkModal isDarkMode={isDarkMode} height={'100%'} top={'0'} right={'0'} width={'100%'} closeFunc={() => setIsNetwork(false)}/>}
      {isMyPlayers && <MyPlayers closeFunc={() => setIsMyPlayers(!isMyPlayers)} height={'100%'} top={'0'} left={'0'}/>}
      {isEvents && <Events height={'100%'} closeFunc={()=>setIsEvents(false)} isDarkMode={isDarkMode} isNotification={isNotification} setIsNotification={setIsNotification} />}
    </div>
  );
};
